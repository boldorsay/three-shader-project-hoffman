...
const vec3 col2 = vec3(0.5, 0.2, 0.8);
const vec3 col3 = vec3(1.0, 0.7, 0.2);
...
void main() {
  ...
  vec3 col = palette(u_time + cos(m_uv.x + m_uv.y), col1, col2, col2, current_color);
  ...
  col = mix(col, shift_col * col + (dist * MOUSE_BRIGHTNESS), dist);
  col = hueShift(col, HUE);
  col *= BRIGHTNESS;

  float bw_col = dot(col, vec3(0.333));
  col = mix(col, vec3(bw_col), BW);
  if (u_mode > 0.5) col = vec3(1.0) - col;
