...
void main() {
    ...
    float noise = snoise(vec3(noiseCoord.x + u_time * 4.0, noiseCoord.y, u_time * 10.)); 
    ...
    for(int i = 0; i < 5; i++) {
      ...
      float noiseCeiling = 0.3 + float(i) * 0.07;


      float noise= 
      smoothstep(noiseFloor, noiseCeiling, 
      snoise(
          vec3(
      ...
      vColor = mix(vColor, u_palette[i], noise);
